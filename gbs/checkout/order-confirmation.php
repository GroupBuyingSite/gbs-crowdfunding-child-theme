<table class="confirmation_table purchase_table" width="100%">
	<thead>
		<tr>
			<th scope="col" colspan="2" class="cart-name gb_ff contrast font_medium"><?php gb_e('Your Order Summary'); ?></th>
			<th scope="col" colspan="1" class="cart-quantity gb_ff contrast font_medium"><?php gb_e('Quantity'); ?></th>
			<th scope="col" colspan="1" class="cart-price gb_ff contrast font_medium"><?php gb_e('Price'); ?></th>
		</tr>
	</thead>
	<tfoot>
		<?php if ( $shipping > 0 ): ?>
				...
				<?php if ( !$all_vouchers_active ): // only show if necessary ?>
					<?php if ( Group_Buying_Purchase::POST_TYPE != get_query_var('post_type') ): // Don't show on purchase template ?>
						<?php if ( !is_user_logged_in() || ( function_exists( 'gb_is_user_guest_purchaser' ) && gb_is_user_guest_purchaser() ) ) : // Don't show if the user is logged in and a guest user ?>
							...
							<p class="message"><strong><?php gb_e( 'Some of your order is pending.' ); ?></strong>  <?php gb_e( 'Save this url to retrieve your invoice(s) later:' ); ?> <a href="<?php echo $lookup_url; ?>"><?php echo $lookup_url; ?></a></p>
						<?php endif ?>
					<?php endif; ?>
				<?php endif ?>
				...
				<?php foreach ( $deal_and_vouchers as $deal_id => $vouchers ): ?>
					<div class="voucher_post clearfix">
						...
						<div class="my_deals_details clearfix"><!-- Begin .my_deals_details -->
							...
							<h2 class="section_heading prime contrast gb_ff"><a href="<?php echo get_permalink( $deal_id ); ?>" title="<?php echo get_the_title( $deal_id ) ?>"><?php echo get_the_title( $deal_id ); ?></a></h2>
							...
							<div class="voucher_left">
								...
								<p class="all_caps"><a href="<?php echo get_permalink( $deal_id ); ?>" title="<?php echo get_the_title( $deal_id ); ?>" class="button"><?php gb_e('View Project') ?></a></p>
								
								
				       			<?php if ( gb_has_merchant_name( $deal_id ) ): ?>
									<br/>
									<p class="merchant_link font_xx_small all_caps"><a href="<?php gb_merchant_url($deal_id) ?>" class="button contrast_button"><?php gb_e('Sponsor Info') ?></a></p>
								<?php endif ?>
